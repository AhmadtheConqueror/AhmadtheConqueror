<h1>Hi, I'm Ahmad! <br/>
<a href="#">Full Stack Web Developer</a>, 
<a href="#">System Analyst</a>
</h1>

<h2>👨‍💻 Software Development Projects:</h2>

- <b>Inventory Management System</b>
  - Flask-based inventory system for managing products, sales, expenses, debts, customers, receipts, and reports.
  - Includes product tracking, cart checkout, invoice generation, financial reports, and database persistence.

- <b>University Clearance System</b>
  - A web-based clearance portal for graduating students.
  - Supports student registration, sequential departmental approvals, staff dashboards, clearance progress tracking, and final clearance form generation.

- <b>Job CV / Job Matching Application</b>
  - A full-stack web application focused on user authentication, profile management, and job matching features.
  - Designed to help users create profiles and explore job opportunities through a clean and responsive interface.

- <b>C# (.NET Projects)</b>
  - [Project Placeholder 1](#)
  - [Project Placeholder 2](#)
  - [Project Placeholder 3](#)

<h2>📊 System Analysis & Design:</h2>

- Use Case Diagrams
- Activity Diagrams
- Entity Relationship Diagrams
- Software Requirements Specification (SRS)
- System Design Documentation
- Database Design
- Workflow and Process Modeling

<h2>📜 Certifications:</h2>

- <b>Getting Started with Microsoft Excel — Coursera</b>  
  <a href="./certificates/Coursera LWZKZCMRQNBW.pdf">View Certificate</a>

- <b>Work Smarter, Not Harder: Time Management for Personal & Professional Productivity — Coursera</b>  
  <a href="./certificates/Coursera RWBUDMH4P5K9.pdf">View Certificate</a>

- <b>Creating a Portfolio Using Weebly — Coursera</b>  
  <a href="./certificates/Coursera RW4NKKTHQCMU.pdf">View Certificate</a>
<h2> 🤳 Connect with me:</h2>

[<img align="left" alt="Ahmad | LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />][linkedin]
[<img align="left" alt="Ahmad | GitHub" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/github.svg" />][github]
[<img align="left" alt="Ahmad | Instagram" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/instagram.svg" />][instagram]
[<img align="left" alt="Ahmad | Twitter" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/twitter.svg" />][twitter]

<br/>
<br/>

[twitter]: https://twitter.com/AhmadIs82802834
[github]: https://github.com/AhmadtheConqueror
[instagram]: https://www.instagram.com/iam_ahmadd._/
[linkedin]: https://www.linkedin.com/in/ahmad-isah-bello-142214283
